namespace Exato.Shared.Features.Cross.GetTwoFactorAuthenticationKey;

public class GetTwoFactorAuthenticationKeyOut : IApiDto<GetTwoFactorAuthenticationKeyOut>
{
    /// <summary>
    /// Chave 2FA do usuário.
    /// </summary>
    public string Key { get; set; }

    /// <summary>
    /// QrCode em Base64 contendo a chave 2FA do usuário.
    /// </summary>
    public string QrCodeBase64 { get; set; }

    /// <summary>
    /// Se o 2FA do usuário está habilitado ou não.
    /// </summary>
    public bool TwoFactorEnabled { get; set; }

    public static IEnumerable<(string, GetTwoFactorAuthenticationKeyOut)> GetExamples() =>
    [
        ("Exemplo", new()
        {
            TwoFactorEnabled = true,
            Key = "COZF2TE2BEWGHEB77A5THFYHPBC2KHPM",
            QrCodeBase64 = @"data:image/png;base64,",
        }),
    ];
}
