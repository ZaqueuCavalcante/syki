...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  loading.value = true
  try {
    await $fetch(`${config.public.backendUrl}/periods/enrollment`, {
      method: 'POST',
      body: event.data,
      credentials: 'include',
    })
    toast.add({ title: 'Período de matrícula criado com sucesso', color: 'success' })
    open.value = false
    emit('created')
  } catch (err: unknown) {
    const msg = (err as { data?: { message?: string } })?.data?.message ?? 'Erro ao criar período de matrícula.'
...
<template>
  <UModal
    v-model:open="open"
    title="Novo período de matrícula"
    :fullscreen="isMobile"
    description="Preencha os dados para cadastrar um novo período de matrícula."
  >
    <template #body>
      <UForm
        :schema="schema"
      ...
      >
        <UFormField label="Nome" name="name">
          <UInput v-model="formState.name" class="w-full" placeholder="Ex: Matrículas 2024.1" />
        </UFormField>
        ...
        <div class="flex justify-end gap-2 pt-2">
          ...
          />
          <UButton
            label="Criar período de matrícula"
            type="submit"
            :loading="loading"
          />
        </div>
