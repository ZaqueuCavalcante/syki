    ...
    ReleaseClassesForEnrollmentService releaseClassesForEnrollmentService) : ICommandHandler<SeedInstitutionEnrollmentsCommand>
{
    public async Task Handle(Guid commandId, SeedInstitutionEnrollmentsCommand command)
    {
        ...
        await releaseClassesForEnrollmentService.Release(id, new() { Classes = classes });
        ...
        var students = await ctx.Students.Where(x => x.CourseOfferingId == adsCourseOfferingId).Select(x => x.Id).ToListAsync();
        foreach (var student in students)
        {
            await createStudentEnrollmentService.Create(id, student, command.AdsCourseCurriculumId, new() { Classes = classes });
        }

        await updateEnrollmentPeriodService.Update(id, academicPeriod, new() { StartAt = new DateTime(today.Year, 3, 3).ToDateOnly(), EndAt = new DateTime(today.Year, 3, 7).ToDateOnly() });

        await startClassesService.Start(id, new() { Classes = classes });
