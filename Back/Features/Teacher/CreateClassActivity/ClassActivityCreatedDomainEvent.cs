...
public record ClassActivityCreatedDomainEvent(Guid ClassId) : IDomainEvent;
...
public class ClassActivityCreatedDomainEventHandler(SykiDbContext ctx) : IDomainEventHandler<ClassActivityCreatedDomainEvent>
{
    ...
    {
        ctx.AddCommand(institutionId, new CreateNewClassActivityNotificationCommand(evt.ClassId), eventId: eventId);
