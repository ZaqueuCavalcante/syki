...
public class GetStudentEnrollmentClassesService(SykiDbContext ctx) : IStudentService
{
    public async Task<List<EnrollmentClassOut>> Get(Guid institutionId, Guid userId, Guid courseCurriculumId)
    {
        ...
        var classes = await ctx.Classes.AsNoTracking()
            ...
            .Include(t => t.Teacher)
            .Where(t => t.InstitutionId == institutionId && t.PeriodId == enrollmentPeriod.Id && (t.Status == ClassStatus.OnEnrollment || t.Status == ClassStatus.Started) && ids.Contains(t.DisciplineId))
            .ToListAsync();
