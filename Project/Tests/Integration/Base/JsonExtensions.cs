...
{
    public static async Task AssertBadRequest(this HttpResponseMessage httpResponse, ExatoError ExatoError)
    {
        var error = await httpResponse.DeserializeTo<ErrorOut>();
        httpResponse.StatusCode.Should().Be(HttpStatusCode.BadRequest);
        error.Message.Should().Be(ExatoError.Message);
        error.Code.Should().Be(ExatoError.Code);
    }

    public static void ShouldBeError<S>(this OneOf<S, ErrorOut> oneOf, ExatoError expected)
    {
        if (oneOf.IsSuccess)
        {
            throw new InvalidOperationException($"Expected error '{expected.Serialize()}' not found");
        }
    ...
    public static void ShouldBeError<S>(this OneOf<S, ExatoError> oneOf, ExatoError expected)
    {
        if (oneOf.IsSuccess)
        {
            throw new InvalidOperationException($"Expected error '{expected.Serialize()}' not found");
